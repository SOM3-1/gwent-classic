class DeckMaker {
    ...
    makeBank(faction, deck) {
        this.clear();
        let cards = card_dict.map((c, i) => ({ card: c, index: i })).filter(p => [faction, "neutral", "weather", "special"].includes(p.card.deck) && p.card.row !== "leader");
    ...
    async runPvPRedrawFlow(state) {
        ...
        try {
            ...
            if (latestState.self.redrawRemaining > 0) {
                ...
                await ui.queueCarousel(player_me.hand, latestState.self.redrawRemaining, async (_container, _index, selectedCard) => {
                ...
                }, () => true, true, true, "Choose up to 2 cards to redraw.");
                this.pvpSelectorActive = false;
            }
    ...
    async runPvPMedicChoiceFlow(state) {
        ...
        try {
            this.pvpSelectorActive = true;
            await ui.queueCarousel(container, 1, async (_graveContainer, _index, selectedCard) => {
            ...
            }, () => true, false, false, "Choose one card from your graveyard to play.");
            this.pvpSelectorActive = false;
            this.flushDeferredPvPState();
        }
        catch (e) {
    ...
    async runPvPDecoyChoiceFlow(state) {
        ...
        try {
            this.pvpSelectorActive = true;
            await ui.queueCarousel(container, 1, async (_rowContainer, _index, selectedCard) => {
                let card = selectedCard;
                let nextState = await this.multiplayerService.sendMatchAction({
                    playerId: this.playerProfile.id,
                    matchId: game.activeMatchId,
                ...
                this.pendingPvPChoiceKey = null;
                this.renderActiveMatchState(nextState);
            }, () => true, false, false, "Choose one unit on your side to swap with Decoy.");
            this.pvpSelectorActive = false;
            this.flushDeferredPvPState();
        }
        catch (e) {
    ...
    async runPvPScoiataelChoiceFlow(state) {
        ...
        this.pendingPvPChoiceKey = choiceKey;
        try {
            let goFirst = await this.showConfirm("The Scoia'tael faction perk allows you to decide who will get to go first.", "Scoia'tael", "Go First", "Let Opponent Start");
            let nextState = await this.multiplayerService.sendMatchAction({
                playerId: this.playerProfile.id,
                matchId: game.activeMatchId,
                action: "resolve_choice",
    ...
    async runPvPLeaderWeatherChoiceFlow(state) {
        ...
        try {
            this.pvpSelectorActive = true;
            await ui.queueCarousel(container, 1, async (_deckContainer, _index, selectedCard) => {
                let card = selectedCard;
                let nextState = await this.multiplayerService.sendMatchAction({
                    playerId: this.playerProfile.id,
                    matchId: game.activeMatchId,
                ...
                this.pendingPvPChoiceKey = null;
                this.renderActiveMatchState(nextState);
            }, () => true, false, false, "Choose one weather card from your deck.");
            this.pvpSelectorActive = false;
            this.flushDeferredPvPState();
        }
        catch (e) {
    ...
    async runPvPLeaderDiscardChoiceFlow(state) {
        ...
        try {
            this.pvpSelectorActive = true;
            await ui.queueCarousel(container, 1, async (_handContainer, _index, selectedCard) => {
                let card = selectedCard;
                let nextState = await this.multiplayerService.sendMatchAction({
                    playerId: this.playerProfile.id,
                    matchId: game.activeMatchId,
                ...
                this.pendingPvPChoiceKey = null;
                this.renderActiveMatchState(nextState);
            }, () => true, false, false, "Choose " + choice.remainingCount + " card" + (choice.remainingCount === 1 ? "" : "s") + " to discard.");
            this.pvpSelectorActive = false;
            this.flushDeferredPvPState();
        }
        catch (e) {
    ...
    async runPvPLeaderDeckToHandChoiceFlow(state) {
        ...
        try {
            ...
            await ui.queueCarousel(container, 1, async (_deckContainer, _index, selectedCard) => {
                ...
                this.pendingPvPChoiceKey = null;
                this.renderActiveMatchState(nextState);
            }, () => true, false, false, "Choose one card from your deck to draw.");
            this.pvpSelectorActive = false;
            this.flushDeferredPvPState();
        }
        catch (e) {
